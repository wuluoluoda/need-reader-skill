...
| `need-reader-clear` | Mark completed Need cards green | `done:` / `done：` |
...
- Parent and child Need completion must not cascade in either direction.
...
2. Run `need-reader-clear` after adding `done:` markers inside completed Need cards.
...
- `need-reader-clear` only changes Need title balls from `🔴` to `🟢`; it does not answer new `need:` notes.
