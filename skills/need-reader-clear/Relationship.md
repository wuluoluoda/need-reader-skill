...
| `need-reader-clear` | Mark completed Need cards green | `done:` / `done：` / `donest:` |
...
- `done:` is local; `donest:` clears the whole parent/current/child Need group.
...
2. Run `need-reader-clear` after adding `done:` or `donest:` markers inside completed Need cards.
...
- `need-reader-clear` changes Need title balls from `🔴` to `🟢`; `done:` clears one card, and `donest:` clears the parent/current/child group.
